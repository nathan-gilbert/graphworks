...
It provides thin conversion helpers between :data:`~graphworks.types.AdjacencyMatrix` (``list[
list[int]]``) and ``numpy.ndarray`` so callers who already have numpy arrays can pass them
directly to :class:`~graphworks.graph.Graph`.

Import pattern — always guard with :data:`TYPE_CHECKING` or a try/except so that code using
graphworks does not *require* numpy::
...
try:
    ...
    msg = "numpy is required for numpy interop. Install it with: pip install graphworks[matrix]"
    raise ImportError(msg) from exc


def ndarray_to_matrix(arr: NDArray) -> AdjacencyMatrix:
    """Convert a numpy ndarray to a stdlib :data:`~graphworks.types.AdjacencyMatrix`.
    ...
    """
    if arr.ndim != 2 or arr.shape[0] != arr.shape[1]:  # noqa: PLR2004
        msg = f"Expected a square 2-D array, got shape {arr.shape}"
        raise ValueError(msg)
    return [[1 if int(val) > 0 else 0 for val in row] for row in arr]


def matrix_to_ndarray(matrix: AdjacencyMatrix) -> NDArray:
    """Convert a stdlib :data:`~graphworks.types.AdjacencyMatrix` to a numpy ndarray.
